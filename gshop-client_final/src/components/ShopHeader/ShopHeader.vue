...
重点:
1.初始显示异常
  情况1: Cannot read property 'a' of undefined"
  原因: 因为a初始值是一个空对象{}, 内部没有数据。当模块中显示的是2级表达式即a.xxx,则不会报错；而当模块中显示3层表达式时，则a.b.xxx就会报错。
  解决: 使用v-if指令
  情况2: Cannot read property 'xxx' of null"
  原因:初始值为null，而模块中直接显示2层表达式，就会报错。
  解决：初始值为{}
2.vue transition动画——给两个弹出的详情框添加动画效果？以商家明细信息页面为例子
步骤:
(1)给商家明细信息页面整体结构增加<transition name="fade"></transition>标签；
(2)在 <div class="shop-brief-modal" v-show="shopShow">同级样式上添加显示和隐藏样式。
(3)添加显示时候的样式:
...
<template>
  <div class="shop-header">
    ...
    <!-- 商家主体内容 -->
    <!-- toggleShopShow()点击事件，实现商品详情的显示和隐藏。 -->
    ...
    <!--商家优惠活动  -->
    <!-- 难点:info.supports[0].type在解析时候console会报错，因为info初始值是{}，所以info.suooprts==undefined.为了解决当页面异步加载数据未完成时，控制台不报错，可以使用v-if指令，只有当info中有support数组时才解析显示，否则就删除。 -->
    ...
    <transition name="fade">
      <div class="shop-brief-modal" v-show="shopShow">
        <div class="brief-modal-content">
          ...
          <!-- toggleShopshow()再点击一下，shopShow就会取反。 -->
